...
# todos: 
#### add in memory constraints -- see if memory constraints exist for all OTHERIND_total_energy parameter combos
#### investigate if need rcp tag for values rcp specific pricing scenarios
...
## / parameter:price_scen / options:price014, price0, price03, WITCHGLOBIOM42_rcp45, WITCHGLOBIOM42_rcp85, REMINDMAgPIE1730_rcp85, REMINDMAgPIE1730_rcp45, REMIND17CEMICS_rcp85, REMIND17CEMICS_rcp45, REMIND17_rcp85, REMIND17_rcp45, MERGEETL60_rcp85, MERGEETL60_rcp45, price014, integration / required:no /
...
get_input_file() {
	...
	input_file=${stem}${as_tag}${price_scen_tag}${geo_level_tag}
        ...
        if [[ -z ${input_file} ]]; then
                echo "Aborting input file was not defined"
                exit 1
        fi
}
