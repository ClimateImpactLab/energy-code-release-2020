...
calculate.beta = function(
    ...
    response = calculate_response(betas.expanded, temp, rel_temp, inc_adapt)
...
plot.beta.maps = function(
    ...
    if (is.null(betas.plot))
        betas.plot = calculate.beta(age, yearlist=yearlist, summ_temp=summ_temp, ...)
    ...
    for (yr in yearlist){
        ...
        filename = glue("{outputwd}/beta_map_{age}_{yr}")
        ...
        if (grey_uclip) {
            ...
            # Update filename.
            filename = glue("{outputwd}/beta_map_{age}_{yr}_grey-uclip")
        }
        ...
        caption_val = glue("Min in {yr}: {minval}    Max in {yr}:  {maxval}    Grey: Out of Sample")
        if(grey_uclip) 
...
plot.spaghetti = function(
    ...
    if (is.null(betas.plot))
        betas = calculate.beta(age, baseline=2010,
            yearlist=c(2010), summ_temp=NULL, ...)
    ...
    if (insample) {
        ...
        #recode outofsample IRs to NA
        df$betas_all_clip = ifelse(!(substr(df$hierid, 1,3) %in% insamplelist), 
            NA, df$betas_all_clip)
    ...
    ggsave(p, filename=glue("{outputwd}/{region}_spaghetti_response_{age}.{ftype}"),
        width = 7, height = 7)  
...
plot.delta.maps = function(
    ...
    filename = glue("{outputwd}/delta_map_{age}_{future}_sensitivity.png")
