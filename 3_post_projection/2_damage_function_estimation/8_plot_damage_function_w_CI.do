...
loc fuel "total_energy_price014"
	preserve
	...
	* Nonparametric model for use pre-2100 
	...
	loc gr `gr' line yhat_`fuel'_2099 anomaly if year == 2099 , yaxis(1) lc(black) lwidth(medthick) ||

	loc gr `gr' rarea y95_`fuel'_`yr' y05_`fuel'_`yr' anomaly if year == 2099 , col(grey%5) ||
	
	di "Graphing time..."
	sort anomaly
	graph twoway `gr', yline(0, lwidth(vthin)) ytitle(`ytitle') xtitle("GMST Anomaly") legend(order(1 "RCP 8.5" 2 "RCP 4.5" 3 "2099 damage fn." 4 "5%-95% ci") size(*0.5)) name("`fuel'", replace) xscale(r(0(1)10)) xlabel(0(1)10) scheme(s1mono) title("`title' Damage Function, End of Century", tstyle(size(medsmall)))  yscale(r(`ymin'(`ystep')`ymax')) ylabel(`ymin'(`ystep')`ymax')  
	        
	capture drop vbl
	
	graph export "$output/fig_3/fig_3C_damage_function_`fuel'_2099_SSP3_w_CI.pdf", replace 
	restore
}
