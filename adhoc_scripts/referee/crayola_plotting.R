...
g1 <- ggplot() +
  ...
  xlab("Time") +
  scale_x_continuous(breaks = seq(2085,2105,10), limits = c(2085, 2106)) +
...
plot_crayola = function(df, avrg_val, output){
  ...
  ggsave(p, filename = glue("{output}/bin_specific_plots/bin_{avrg_val_string}.png"), width = 8, height = 8)
}
