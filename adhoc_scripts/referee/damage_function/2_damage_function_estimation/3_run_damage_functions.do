 ...
 */	* Generate predicted coeffs for each year post 2100 with linear extrapolation
	foreach yr of numlist 2100/2300 {
		di "`vv' `yr'"
		* CHECK: 
		loc cons = _b[_cons] + _b[t] * (`yr'-2010) + _b[c.t#c.t] * ((`yr'-2010)^2)
		loc beta1 = _b[anomaly] + _b[c.anomaly#c.t]*(`yr'-2010) + _b[c.anomaly#c.t#c.t]*((`yr'-2010)^2)
		loc beta2 = _b[c.anomaly#c.anomaly] + _b[c.anomaly#c.anomaly#c.t]*(`yr'-2010) + _b[c.anomaly#c.anomaly#c.t#c.t]*((`yr'-2010)^2) 
...
outsheet using "$dir_output/coefficients/df_mean_output_`ssp'`model_tag'.csv", comma replace	
