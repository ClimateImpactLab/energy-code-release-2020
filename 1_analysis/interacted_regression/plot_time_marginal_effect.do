...
forval lg = 1/3 {
	...
	forval k=1/2 {
		local line " `line' `add' _b[c.indp`pg'#c.indf1#c.FD_year2temp`k'_GMFD] * (temp`k' - 20^`k') * year * 2"
		local line "`line' + _b[c.indp`pg'#c.indf1#c.FD_dc1_lgdppc_MA15year2I`ig'temp`k'] * `deltacut_subInc' * (temp`k' - 20^`k') * year * 2"
		local add " + "
	}
	** trace out dose response marginal effect
	predictnl yhat`lg' = `line', se(se`lg') ci(lower`lg' upper`lg')
