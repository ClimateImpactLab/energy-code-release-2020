...
//load data and set values for overlay
	...
	use "`data'/`clim_data'/rationalized_code/$data_type/data/clim`clim_data'_`case'`IF'_`bknum'_visual_`model'_`data_type'_regsort.dta", replace
	...
	foreach tag in "OTHERIND" {
		...
		loc nonSE ""
						...
						*loop over the polynomials' degree and save the predict command in the local `line'
							...
							estimates use "`ster'/`clim_data'/rationalized_code/$data_type/sters/`FD'`FGLS'/quant`t'`qt'_`FD'`FGLS'_inter_clim`clim_data'_`case'`IF'_`bknum'_poly`o'_`model'.ster"
							
							//predict
							predictnl yhat_`clim_data' = `line', se(se_`clim_data') ci(lower_`clim_data' upper_`clim_data')
					
					loc SE = "`SE' rarea upper_`clim_data' lower_`clim_data' temp1, col(`col'%30) || line yhat_`clim_data' temp1, lc (`col') ||"
					loc noSE "`noSE' line yhat_`clim_data' temp1, lc (`col') ||"
		...
		cap mkdir "`output'/quantile_plots"	
		cap mkdir "`output'/quantile_plots/`tag'_`var'"	
						...
						title("Poly `o' `qt' Quantile Model for `tit' `stit' (`model')", size(small)) ///
						subtitle("`colorGuide'", size(small)) ///
						plotregion(color(white)) graphregion(color(white)) name(comb, replace)
						graph export "`output'/quantile_plots/`tag'_`var'/quant`t'`qt'_`FD'`FGLS'_inter_poly`o'_`model'_`tag'_`var'_clim`clim_data'_`case'`IF'_`bknum'.pdf", replace
						...
						graph export "`output'/quantile_plots/`tag'_`var'/quant`t'`qt'_`FD'`FGLS'_inter_poly`o'_`model'_`tag'_`var'_clim`clim_data'_`case'`IF'_`bknum'_noSE.pdf", replace
