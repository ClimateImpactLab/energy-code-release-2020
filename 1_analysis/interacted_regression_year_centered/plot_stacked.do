...
if ( "`submodel_ov'" == "EX"  ) {
	preserve
	use "$DATA/regression/GMFD/break_data_`model_main'_`submodel_ov'.dta", clear
	summ maxInc_largegpid_`var' if largegpid_`var' == 1
	local ibar_ov = `r(max)'
	restore
}
...
graph export "$OUTPUT/figures/`fig'_`var'_interacted_`plot_title'_cyear.pdf", replace
