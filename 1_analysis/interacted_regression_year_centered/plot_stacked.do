		...
		foreach type in `type_list' {
			...
			foreach k of num 1/2 {
				...
				if ((strpos("`plot_model'", "lininter") > 0) | (strpos("`plot_model'", "quadinter") > 0)) {
					local line = "`line' + _b[c.indp`pg'#c.indf`fg'#c.FD_cyeartemp`k'_GMFD] * (temp`k' - 20^`k')*`cyear'"
					local line = "`line' + _b[c.indp`pg'#c.indf`fg'#c.FD_dc1_lgdppc_MA15cyearI`ig'temp`k']*`deltacut_subInc'*`cyear'*(temp`k' - 20^`k')"
				}

				if (strpos("`plot_model'", "quadinter") > 0) {
					local line = "`line' + _b[c.indp`pg'#c.indf`fg'#c.FD_cyear2temp`k'_GMFD] * (temp`k' - 20^`k')*`cyear'*`cyear'"
					local line = "`line' + _b[c.indp`pg'#c.indf`fg'#c.FD_dc1_lgdppc_MA15cyear2I`ig'temp`k']*`deltacut_subInc'*`cyear'*`cyear'*(temp`k' - 20^`k')"
				}
...
graph export "$root/figures/`fig'_`var'_interacted_`plot_title'_cyear.pdf", replace
