...
graph export "$root/figures/fig_1A_electricity_income_decile_`model'.pdf", replace
...
// plot and save combined plot with SE
graph combine `graphic', imargin(zero) ycomm rows(1) xsize(20) ysize(3) ///
title("Poly 2 Income Decile Other Energy Temperature Response (`model')", size(small)) ///
subtitle("`colorGuide'", size(small)) ///
plotregion(color(white)) graphregion(color(white)) name(comb, replace)
graph export "$root/figures/fig_1A_other_energy_income_decile_`model'.pdf", replace
