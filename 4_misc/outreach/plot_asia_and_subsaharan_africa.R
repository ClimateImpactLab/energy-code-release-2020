...
asian_IRs = split$Asia[,region]
mymap_asia = mymap %>% dplyr::filter(as.character(id) %in% asian_IRs)



plot_2A = function(fuel, bound, DB_data, map=mymap_asia) {
  ...
  df = df %>% dplyr::filter(region %in% asian_IRs)
  ...
  ggsave(paste0(output, "/fig_2A_", fuel, "_impacts_map_asia.pdf"), p)
}
...
############
# plot russia as it's not included in Asia

russia_IRs = (mymap %>% dplyr::filter(substr(id, 1,3) =="RUS"))$id
mymap_russia = mymap %>% dplyr::filter(as.character(id) %in% russia_IRs)

plot_2A = function(fuel, bound, DB_data, map=mymap_russia) {
  ...
  df = df %>% dplyr::filter(region %in% russia_IRs)
  # Set scaling factor for map color bar
  scale_v = c(-1, -0.2, -0.05, -0.005, 0, 0.005, 0.05, 0.2, 1)
  rescale_value <- scale_v*bound
  ...
  p = join.plot.map(map.df = map, 
                     ...
                     map.title = paste0(fuel, 
                                    "_TINV_clim_SSP3-rcp85_impactpc_high_fulladapt_2099"))
  ggsave(paste0(output, "/fig_2A_", fuel, "_impacts_map_russia.pdf"), p)
}
plot_2A(fuel = "electricity", bound = 3, DB_data = DB_data)
plot_2A(fuel = "other_energy", bound = 18, DB_data = DB_data)
