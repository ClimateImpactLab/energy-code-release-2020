...
foreach price in `pricelist' {
	di "`price'"
	import delim "$dir/resampled_data/gcm_damages_OTHERIND_total_energy_`price'_`ssp'`model_tag'-100-draws_noadapt.csv", clear case(preserve)
...
use "`coeffs'", clear
outsheet using "$dir/coefficients/df_qreg_output_`ssp'`model_tag'_noadapt.csv", comma replace
