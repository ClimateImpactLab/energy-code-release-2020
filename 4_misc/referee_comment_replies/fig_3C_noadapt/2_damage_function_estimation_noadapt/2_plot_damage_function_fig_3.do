...
foreach fuel in "total_energy_price014" "electricity" "other_energy" {
	...
	import delim using "$dir/gcm_`type'_OTHERIND_`fuel'_SSP3-15-draws_noadapt.csv", clear
	drop if year < 2015 | year > 2099
...
foreach fuel in "total_energy_p14" "other_energy" "electricity" {
	...
	* Display the slope of this damage function in 2099
	...
	graph export "$output/fig_3/fig_3C_damage_function_`fuel'_2099_SSP3_noadapt.pdf", replace 
	restore
}
...
graph export "$output/fig_3/fig_3C_anomaly_densities_GMST_end_of_century_noadapt.pdf", replace 
graph drop _all
