...
global DB = "/mnt/"
glob coefs_dir = "$DB/CIL_energy/code_release_data_pixel_interaction/referee_comments/damage_function_estimation/coefficients"
...
foreach subset in 2010 2050 2085 {
	import delimited "$coefs_dir/df_mean_output_SSP3_percent_gdp_`subset'.csv", varnames(1) clear
	...
	* Save bin level trends as a csv 
	postclose dtrend
	use "`dtrend'", clear
	export delimited using "$output/level_trend_v2_percent_gdp_`subset'.csv", replace

	loc expa2 = 2110 - `subset' + 1
	expand `expa2', gen(newobs)
	bysort model avrg: gen year = `subset' + (_n-1)
	gen t = year-2010
	...
	* Append information on "yh", which is the actual level of the df in each bin for 2085 to 2100
	...
	export delimited using "$output/crayola_level_v3_percent_gdp_`subset'.csv", replace
