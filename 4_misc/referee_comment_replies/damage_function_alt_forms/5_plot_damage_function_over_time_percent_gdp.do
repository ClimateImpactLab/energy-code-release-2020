...
foreach subset in 2085 2050 2010 {
	...
	* Load in damage function coefficients, and subset to just price014 (main model) 
	insheet using "$dir_output/coefficients/df_mean_output_SSP3_percent_gdp_`subset'.csv", comma names clear
	...
	* Plot and save
	graph tw `gr', yline(0, lwidth(vthin)) ytitle("Bn 2019 USD" ) xtitle("GMST Anomaly") title("Total Energy Damage Function, Evolution Over Time, Subset `subset'~", size(small)) xscale(r(0(1)10)) xlabel(0(1)10) legend(off) scheme(s1mono) ylabel(, labsize(small)) 

	graph export "$output/fig_Appendix-E1_total_energy_percent_gdp_damage_function_evolution_SSP3-price014_`subset'.pdf", replace 
	graph drop _all
}
